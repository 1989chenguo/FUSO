...
/* TCP fastopen related information */
	...
	//CG add for remember if this flow has all been protected.
	///If only protection packets in this sub-flow, this field is also set
	int ifAllProtected;
	u32	total_trans; //// total data packet sent including retrans
	u32 packet_sent_from_last_retran; /// total data packet sent including retrans from last retran
	struct sk_buff *next_protected_packet;  //the next packet going to be protected

	u32 total_recovery_trans;
	u32 total_trans_from_last_read;
	u32 total_retrans_from_last_read;
	u32 total_recovery_trans_from_last_read;

	int ifProactive; ///Flag to state whether this sk is proactive enalbed
	////CG add end
