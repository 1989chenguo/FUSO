...
//CG add
// #define no_RMT_IP "172.22.80.224"
#define no_RMT_IP "192.168.13.64"
#define NIPQUAD(addr) \
...
struct mptcp_cb {
	...
	int ifAllDataHasBeenSent;///CG add to identify if all the data for this mptcp connection has all been sent out.
...
/*CG add*/
extern int sysctl_mptcp_rmt;
extern int sysctl_mptcp_init_subflow_once;
extern int sysctl_cg_drop_rate; ///CG defined sysctl_cg_drop_rate/10000= drop rate
extern int sysctl_mptcp_rmt_path_selection; ////2 for loss rate selection, others for default cost metric
extern int sysctl_mptcp_receive_ofo_optimize; 
/*CG add*/
...
/*mengyuan*/
extern bool try_send_recoveries(struct sock *meta_sk); ///CG add   
/*mengyuan*/
...
static inline int mptcp_pi_to_flag(u8 pi)
{
	return 1 << (pi - 1);
}
