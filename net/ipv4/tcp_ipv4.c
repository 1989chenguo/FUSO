...
int tcp_v4_rcv(struct sk_buff *skb)
{
	...
	// //CG add
	// if(sysctl_cg_drop_rate>0 && (ntohs(iph->tot_len) - th->doff * 4 >0))
	// {
	// 	int i, lessthan10000;
	// 	get_random_bytes(&i, sizeof(i));
	// 	lessthan10000 = i % 10000;
		
	// 	if(lessthan10000<sysctl_cg_drop_rate)
	// 	{
	// 		char srcIP[16],dstIP[16];
	// 		memset(srcIP,'\0',16);
	// 		memset(dstIP,'\0',16);
	// 		sprintf(srcIP,"%d.%d.%d.%d",NIPQUAD(iph->saddr));
	// 		sprintf(dstIP,"%d.%d.%d.%d",NIPQUAD(iph->daddr));
	// 		printk(KERN_EMERG "!!!!!!!!!!!!!!Drop a packet [rnd=%d,trshld=%d] (%s--->%s) iplen=%d, thlen=%d, source = %u, dest = %u, seq = %u ,ack_seq = %u!!!!!!!!!!\n"
	//  		,lessthan10000,sysctl_cg_drop_rate,srcIP,dstIP,ntohs(iph->tot_len),th->doff*4,ntohs(th->source),ntohs(th->dest),ntohl(th->seq),ntohl(th->ack_seq));
 ...
 //    {
	...
	// 		printk(KERN_EMERG "-----tcp_v4_rcv (%s--->%s)\n",srcIP,dstIP);

	//     if(ifRMTed==1)
	// 		printk(KERN_EMERG "++++++++++receive a packet source = %u, dest = %u, seq = %u ,ack_seq = %u+++++++++++\n"
	...
	if (!sock_owned_by_user(meta_sk)) {
		if (!tcp_prequeue(meta_sk, skb))
		{
		   ...
		   //  		char srcIP[16],dstIP[16];
					...
					// sprintf(srcIP,"%d.%d.%d.%d",NIPQUAD(iph->saddr));
					// sprintf(dstIP,"%d.%d.%d.%d",NIPQUAD(iph->daddr));
					// printk(KERN_EMERG "-----tcp_v4_do_rcv (%s--->%s) ifAllProtected=%d ifAllDataHasBeenSent=%d\n",srcIP,dstIP,tcp_sk(meta_sk)->ifAllProtected,tcp_sk(meta_sk)->mpcb->ifAllDataHasBeenSent);
		    		if(!tcp_send_head(meta_sk) && tcp_sk(meta_sk)->mpcb->ifAllDataHasBeenSent==1)
						try_send_recoveries(meta_sk);
		    	}
			}
			////CG add
		}
