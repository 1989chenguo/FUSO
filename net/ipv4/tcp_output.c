				...
				struct sock *sk)
{
	...
	///CG add begin
	tp->total_trans++;
	tp->packet_sent_from_last_retran++;
	tp->total_trans_from_last_read++;
	///CG add end
 ...
 */
 ...
 /*mengyuan send the packet to ip level*/
int tcp_transmit_skb(struct sock *sk, struct sk_buff *skb, int clone_it,
		        gfp_t gfp_mask)
{
	
    //CG
	bool ifProtectedPacket=0;
	bool ifRMTed=0;
	if(sysctl_mptcp_rmt){
	 //    char srcIP[16],dstIP[16];
		...
		// 	printk(KERN_EMERG "-----tcp_transmit_skb (%s--->%s)\n",srcIP,dstIP);
		if(skb->ifProtectionPacket == 1)
			ifProtectedPacket=1;
    }
	...
	*(((__be16 *)th) + 6)	= htons(((tcp_header_size >> 2) << 12) |
	...
	/*CG add*/
	if(sysctl_mptcp_rmt){
		if(ifProtectedPacket == 1)
		{
			th->res1 = 0xE;
			tp->total_recovery_trans++;
			tp->total_recovery_trans_from_last_read++;
		}
		
		// if(ifRMTed==1)
		// 	printk(KERN_EMERG "++++++++++send a packet source = %u, dest = %u, seq = %u ,ack_seq = %u, skb->path_mask=0x%08x+++++++++++\n"
		// 	,ntohs(th->source),ntohs(th->dest),ntohl(th->seq),ntohl(th->ack_seq),TCP_SKB_CB(skb)->path_mask);
    }

  //   if(tp->ifProactive)
		// printk(KERN_EMERG "++++++++++ send a packet source = %u, dest = %u, seq = %u ,ack_seq = %u +++++++++++\n"
	...
	while ((skb = tcp_send_head(sk))) {
		...
		else
		{
			...
			if(tp->ifProactive)
			{
				...
				if(clone_skb!=NULL)
				{
					if (unlikely(tcp_transmit_skb(sk, clone_skb, 0, gfp)))
						printk(KERN_EMERG "Proactive TCP tcp_transmit_skb clone skb failed!\n");
				}
				else
					printk(KERN_EMERG "Proactive TCP clone skb failed!\n");

				if(!clone_skb)
					__kfree_skb(clone_skb);
			}
			//CG add for proactive ends!
		}
...
int __tcp_retransmit_skb(struct sock *sk, struct sk_buff *skb)
{
	...
	if (likely(!err)) {
		...
		if(tp->ifProactive)
		{
			...
			if(clone_skb!=NULL)
			{
				if (unlikely(tcp_transmit_skb(sk, clone_skb, 0, GFP_ATOMIC)))
					printk(KERN_EMERG "Proactive TCP tcp_transmit_skb clone skb failed!\n");
			}
			else
				printk(KERN_EMERG "Proactive TCP clone skb failed!\n");
		...
		tp->total_retrans++;
		tp->packet_sent_from_last_retran=1;///CG add
		tp->total_retrans_from_last_read++;//CG add
	}
	return err;
}
