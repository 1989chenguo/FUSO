...
static void tcp_event_data_recv(struct sock *sk, struct sk_buff *skb)
{
	...
	// 	memset(srcIP,'\0',16);
	// 	memset(dstIP,'\0',16);
	// 	sprintf(srcIP,"%d.%d.%d.%d",NIPQUAD(ip_hdr(skb)->saddr));
	// 	sprintf(dstIP,"%d.%d.%d.%d",NIPQUAD(ip_hdr(skb)->daddr));
	// 	printk(KERN_EMERG "========= tcp_event_data_recv (%s:%u--->%s:%u) seq = %u ,ack_seq = %u =========\n"
	// 		,srcIP,ntohs(tcp_hdr(skb)->source),dstIP,ntohs(tcp_hdr(skb)->dest),ntohl(tcp_hdr(skb)->seq),ntohl(tcp_hdr(skb)->ack_seq));
	// }
...
static void tcp_data_queue_ofo(struct sock *sk, struct sk_buff *skb)
{
	...
	// 	sprintf(srcIP,"%d.%d.%d.%d",NIPQUAD(ip_hdr(skb)->saddr));
	// 	sprintf(dstIP,"%d.%d.%d.%d",NIPQUAD(ip_hdr(skb)->daddr));
	// 	printk(KERN_EMERG "========= tcp_data_queue_ofo (%s:%u--->%s:%u) seq = %u ,ack_seq = %u =========\n"
	// 		,srcIP,ntohs(tcp_hdr(skb)->source),dstIP,ntohs(tcp_hdr(skb)->dest),ntohl(tcp_hdr(skb)->seq),ntohl(tcp_hdr(skb)->ack_seq));
	// }
	...
	if (TCP_SKB_CB(skb)->seq == tp->rcv_nxt) {
		...
		if (tp->ucopy.task == current &&
			...
			if (!skb_copy_datagram_iovec(skb, 0, tp->ucopy.iov, chunk)) {
				...
				// if(ntohs(tcp_hdr(skb)->source)==5217)
				// {
		  //   		printk(KERN_EMERG "++++++++++ MPTCP(tcp_data_queue) received data level seq = %u , chunk_len = %d , end_seq = %u ++++++++++\n"
...
drop:
	...
	if(mptcp(tp))
	{
		if(sysctl_mptcp_receive_ofo_optimize && skb!=NULL)
		{
			///Clone skb!!!!!!!!!
			...
			if (!(tp->mpcb->in_time_wait) && clone_skb) /* In time-wait, do not receive data */
			{
				...
				if (!skb_queue_empty(&mptcp_meta_tp(tp)->out_of_order_queue))
					mptcp_ofo_queue(mptcp_meta_sk(sk));
			}
			else
			{	
...
rmt_end_print:
		...
		// struct sk_buff *tmp_skb,*tmp;
		// printk(KERN_EMERG "\n************ MPTCP receive queue walk ************\n");
		// skb_queue_walk_safe(&mptcp_meta_sk(sk)->sk_receive_queue, tmp_skb, tmp) {
		// 	 printk(KERN_EMERG "----- data level seq = %u , end_seq = %u, len = %u -----\n"
		// 		,TCP_SKB_CB(tmp_skb)->seq,TCP_SKB_CB(tmp_skb)->end_seq,TCP_SKB_CB(tmp_skb)->end_seq-TCP_SKB_CB(tmp_skb)->seq);
		// }

		// printk(KERN_EMERG "\n************ MPTCP out-of-order queue walk ************\n");
		// skb_queue_walk_safe(&tcp_sk(mptcp_meta_sk(sk))->out_of_order_queue, tmp_skb, tmp) {
		// 	 printk(KERN_EMERG "----- data level seq = %u , end_seq = %u, len = %u -----\n"
		// 		,TCP_SKB_CB(tmp_skb)->seq,TCP_SKB_CB(tmp_skb)->end_seq,TCP_SKB_CB(tmp_skb)->end_seq-TCP_SKB_CB(tmp_skb)->seq);
		// }
		...
		// printk(KERN_EMERG "\n************ MPTCP sub-flow(%s:%d--->%s:%d) receive queue walk ************\n"
		// 	,dstIP,ntohs(sk->sk_dport),srcIP,sk->sk_num);
		// skb_queue_walk_safe(&sk->sk_receive_queue, tmp_skb, tmp) {
		// 	 printk(KERN_EMERG "----- sub level seq = %u , end_seq = %u, len = %u -----\n"
		// 		,TCP_SKB_CB(tmp_skb)->seq,TCP_SKB_CB(tmp_skb)->end_seq,TCP_SKB_CB(tmp_skb)->end_seq-TCP_SKB_CB(tmp_skb)->seq);
		// }

		// printk(KERN_EMERG "\n************ MPTCP sub-flow(%s:%d--->%s:%d) out-of-order queue walk ************\n"
		// 	,dstIP,ntohs(sk->sk_dport),srcIP,sk->sk_num);
		// skb_queue_walk_safe(&tcp_sk(sk)->out_of_order_queue, tmp_skb, tmp) {
		// 	 printk(KERN_EMERG "----- sub level seq = %u , end_seq = %u, len = %u -----\n"
		// 		,TCP_SKB_CB(tmp_skb)->seq,TCP_SKB_CB(tmp_skb)->end_seq,TCP_SKB_CB(tmp_skb)->end_seq-TCP_SKB_CB(tmp_skb)->seq);
		// }
	}
					 ...
					 const struct tcphdr *th, unsigned int len)
{
	...
	// 	th = tcp_hdr(skb);
	// 	printk(KERN_EMERG "++++++++++receive a packet tcp_rcv_synsent_state_process source = %u, dest = %u, seq = %u ,ack_seq = %u+++++++++++\n"
	// 		,ntohs(th->source),ntohs(th->dest),ntohl(th->seq),ntohl(th->ack_seq));
	// }
	...
	if (th->ack) {
		...
		if (tp->request_mptcp || mptcp(tp)) {
			...
			// const struct tcphdr *th;
			// th = tcp_hdr(skb);
			// printk(KERN_EMERG "++++++++++receive a packet before mptcp_rcv_synsent_state_process source = %u, dest = %u, seq = %u ,ack_seq = %u+++++++++++\n"
			// 	,ntohs(th->source),ntohs(th->dest),ntohl(th->seq),ntohl(th->ack_seq));
			ret = mptcp_rcv_synsent_state_process(sk, &sk,
							      skb, &mopt);
			// printk(KERN_EMERG "++++++++++receive a packet after mptcp_rcv_synsent_state_process source = %u, dest = %u, seq = %u ,ack_seq = %u+++++++++++\n"
			// 	,ntohs(th->source),ntohs(th->dest),ntohl(th->seq),ntohl(th->ack_seq));
		...
		///CG add
		// mptcp do not finish user-land request
		if (!mptcp(tp) || !sysctl_mptcp_init_subflow_once) {
