...
/*mengyuan sysctl add*/
int sysctl_mptcp_rmt __read_mostly = 0;
int sysctl_mptcp_init_subflow_once __read_mostly = 0;
int sysctl_cg_drop_rate __read_mostly = 0;
int sysctl_mptcp_rmt_path_selection __read_mostly = 2;
int sysctl_mptcp_receive_ofo_optimize __read_mostly = 0;
/*mengyuan */
...
static struct ctl_table mptcp_table[] = {
	...
	{ 
        .procname = "mptcp_rmt",
		.data = &sysctl_mptcp_rmt,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
	{ 
        .procname = "mptcp_init_subflow_once",
		.data = &sysctl_mptcp_init_subflow_once,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
	{ 
        .procname = "cg_drop_rate",
		.data = &sysctl_cg_drop_rate,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
    /*    sysctl_mptcp_rmt_path_selection 
        1 for original estimated draining time
		2  for lossrate comparison*/
	{ 
        .procname = "mptcp_rmt_path_selection",
		.data = &sysctl_mptcp_rmt_path_selection,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
	{ 
        .procname = "mptcp_receive_ofo_optimize",
		.data = &sysctl_mptcp_receive_ofo_optimize,
		.maxlen = sizeof(int),
		.mode = 0644,
		.proc_handler = &proc_dointvec,
	},
...
void mptcp_sub_close_wq(struct work_struct *work)
{
	...
	if (!tp||!sk||!meta_sk) { ///CG add to avoid MPTCP del sock bug
