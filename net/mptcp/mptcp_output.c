...
bool rmt_mptcp_reinject_data(struct sock *sk, int clone_it,struct sk_buff *skb_it, struct sock *dest_sk)
{
	struct tcp_sock *tp = tcp_sk(sk);
	struct sock *meta_sk = tp->meta_sk;
...
bool simple_retransmission(struct sock *worst_sk,struct sock *meta_sk,struct sock *best_sk,struct sk_buff *skb){
    ...
    if(rmt_mptcp_reinject_data(worst_sk,clone_it,skb,best_sk))
    	return 1;
...
bool try_send_recoveries(struct sock *meta_sk)///CG add
{
	//CG
 //    char srcIP[16],dstIP[16];
	// memset(srcIP,'\0',16);
	// memset(dstIP,'\0',16);
	// sprintf(srcIP,"%d.%d.%d.%d",NIPQUAD(meta_sk->sk_rcv_saddr));
	// sprintf(dstIP,"%d.%d.%d.%d",NIPQUAD(meta_sk->sk_daddr));
	// printk(KERN_EMERG "-----try_send_recoveries (%s--->%s)\n",srcIP,dstIP);
	...
	while(1)
	{
		...
		if (!skb)
		{
			// printk(KERN_EMERG "rmt error: worst subflow found, but no protectable un-ACKed packet!\n");
			return 1;
		}
		...
		//CG
	 //    char bestSrcIP[16],bestDstIP[16],worstSrcIP[16],worstDstIP[16];
		// memset(bestSrcIP,'\0',16);
		// memset(bestDstIP,'\0',16);
		// memset(worstSrcIP,'\0',16);
		// memset(worstDstIP,'\0',16);
		// sprintf(bestSrcIP,"%d.%d.%d.%d",NIPQUAD(best_sk->sk_rcv_saddr));
		// sprintf(bestDstIP,"%d.%d.%d.%d",NIPQUAD(best_sk->sk_daddr));
		// sprintf(worstSrcIP,"%d.%d.%d.%d",NIPQUAD(worst_sk->sk_rcv_saddr));
		// sprintf(worstDstIP,"%d.%d.%d.%d",NIPQUAD(worst_sk->sk_daddr));

	 //    printk(KERN_EMERG "going to send a protection  worst(%s:%d-->%s:%d) =====> best(%s:%d-->%s:%d)\n"
		// 	,worstSrcIP,worst_sk->sk_num,worstDstIP,ntohs(worst_sk->sk_dport),bestSrcIP,best_sk->sk_num,bestDstIP,ntohs(best_sk->sk_dport));

	 //    //CG
		// printk(KERN_EMERG "rmt original sub level seq = %u ,ack_seq = %u\n"
		// 	,TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq);

		if(ifLastPacket == true)
		{
			// printk(KERN_EMERG "-----set tcp_sk(worst_sk)->ifAllProtected=1\n");
			tcp_sk(worst_sk)->ifAllProtected=1;
		}
		
		if(simple_retransmission(worst_sk,meta_sk,best_sk,skb))
			return 1;
	}
}
