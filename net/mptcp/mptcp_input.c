				    ...
				    struct sock *meta_sk)
{
	...
	if (!skb_copy_datagram_iovec(skb, 0, meta_tp->ucopy.iov, chunk)) {
		...
		// ///CG add
	 //    printk(KERN_EMERG "++++++++++ MPTCP(mptcp_direct_copy) received data level seq = %u , chunk_len = %d , end_seq = %u ++++++++++\n"
		// 	,meta_tp->copied_seq,chunk,meta_tp->copied_seq+chunk);
...
////CG add
	...
	// skb_queue_walk_safe(&meta_sk->sk_receive_queue, skb, tmp) {
	// 	 printk(KERN_EMERG "----- data level seq = %u , end_seq = %u, len = %u -----\n"
	// 		,TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq,TCP_SKB_CB(skb)->end_seq-TCP_SKB_CB(skb)->seq);
	// }

	// printk(KERN_EMERG "\n************ MPTCP out-of-order queue walk ************\n");
	// skb_queue_walk_safe(&tcp_sk(meta_sk)->out_of_order_queue, skb, tmp) {
	// 	 printk(KERN_EMERG "----- data level seq = %u , end_seq = %u, len = %u -----\n"
	// 		,TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq,TCP_SKB_CB(skb)->end_seq-TCP_SKB_CB(skb)->seq);
	// }
	...
	// 	,dstIP,ntohs(sk->sk_dport),srcIP,sk->sk_num);
	// skb_queue_walk_safe(&tcp_sk(sk)->out_of_order_queue, skb, tmp) {
	// 	 printk(KERN_EMERG "----- sub level seq = %u , end_seq = %u, len = %u -----\n"
	// 		,TCP_SKB_CB(skb)->seq,TCP_SKB_CB(skb)->end_seq,TCP_SKB_CB(skb)->end_seq-TCP_SKB_CB(skb)->seq);
	// }
}
				    ...
				    const struct mptcp_options_received *mopt)
{
	...
	//CG add
	if(sysctl_mptcp_init_subflow_once)
	{
		// printk(KERN_EMERG "-----mptcp_rcv_synsent_state_process\n");
		// tcp_sk(sk)->mptcp->fully_established = 1;
		mptcp_become_fully_estab(sk);

		struct mptcp_cb *mpcb = tcp_sk(mptcp_meta_sk(sk))->mpcb;
		bool all_established = 1;
		mptcp_for_each_sk(mpcb, sk) {
		   if (!tcp_sk(sk)->mptcp->fully_established)
		           all_established = 0;
		}

		if (all_established) {
		   tcp_finish_connect(mptcp_meta_sk(sk), NULL);
		}
	}

	// //CG add
	// // this will be hit when every subflow becomes fully established
	// if(sysctl_mptcp_init_subflow_once)
	// {
	// 	tp->mpcb->pm_ops->fully_established(mptcp_meta_sk(sk));

	// 	struct mptcp_cb *mpcb = tcp_sk(mptcp_meta_sk(sk))->mpcb;
	// 	bool all_established = 1;
	// 	mptcp_for_each_sk(mpcb, sk) {
	// 	   if (!tcp_sk(sk)->mptcp->fully_established  &&  !tcp_sk(sk)->mptcp->pre_established)
	// 	           all_established = 0;
	// 	}
