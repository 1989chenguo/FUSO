					  ...
					  unsigned int *limit)
{
	...
	*limit = 0;
	...
	///CG add
	if(*reinject==1 && skb->ifProtectionPacket==1)////Retrieved from reinject queue, possibly rmtProtectionPacket
	{
		struct sock *sk;
		int ifFoundPath=0;
		struct mptcp_cb *mpcb = tcp_sk(meta_sk)->mpcb;
		mptcp_for_each_sk(mpcb, sk) {
			if((TCP_SKB_CB(skb)->path_mask&mptcp_pi_to_flag(tcp_sk(sk)->mptcp->path_index))==0)
			{	
				*subsk=sk;
				ifFoundPath=1;
				break;
		...
		if(ifFoundPath==0)
			printk(KERN_EMERG "rmt  error: No path found although dest path was set! \n");
	}
	else
		*subsk = get_available_subflow(meta_sk, skb, false);///Original
